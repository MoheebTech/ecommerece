...
class MobileAdminLogin extends StatefulWidget {
  @override
  _MobileAdminLoginState createState() => _MobileAdminLoginState();
}

class _MobileAdminLoginState extends State<MobileAdminLogin> {
  ...
  mobileadminsignin() async {
    try {
      UserCredential userCredential = await _auth.signInWithEmailAndPassword(
          email: emailcontroller.text, password: passcontroller.text);
       var users=await _auth.currentUser;
       AdminStaticVeriable.uid = users!.uid;
        await  firebaseFirestore.collection("Admins").doc(AdminStaticVeriable.uid).get().then(
          (value){
           setState(() {
             if(value.data()==null){
...
//     ///                                        
                                      Padding(
                                        ...
                                        child: TextButton(
                                          onPressed: () {
                                            Navigator.push(context, MaterialPageRoute(builder: (context)=>const MobileAdminSignUp()));
                                          },
                                          child: Text('SignUp',
                                              style: TextStyle(
                                                  fontSize: width * 0.05,
